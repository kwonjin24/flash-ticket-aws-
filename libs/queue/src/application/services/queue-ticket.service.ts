...
import { REDIS_QUEUE_CLIENT } from '../../infrastructure/redis.provider';
...
export class QueueTicketService {
  private readonly logger = new Logger(QueueTicketService.name);
  ...
  async enqueue(
    ...
    await multi.exec();
    this.logger.log(`Enqueued ticket ${ticketId} for user ${userId} (event ${eventId})`);
    return { ticketId };
  }
