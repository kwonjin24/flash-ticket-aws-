  ...
  return (
    <main className="queue-page">
      <div className="queue-page__container">
        <h2 className="queue-page__title">대기열 참여</h2>
        ...
        {selectedEvent && (
          <section className="queue-page__event-info">
            <h3 className="queue-page__event-title">{selectedEvent.name}</h3>
            <p className="queue-page__event-text">{formatDateRange(selectedEvent.startsAt, selectedEvent.endsAt)}</p>
            <p className="queue-page__event-stock">
              재고: {Math.max(selectedEvent.totalQty - selectedEvent.soldQty, 0)} / {selectedEvent.totalQty}
            </p>
            <p className="queue-page__event-limit">1인당 최대 {selectedEvent.maxPerUser}매</p>
        ...
        {typeof queueSize === 'number' && (
          <section className="queue-page__status">
            <h3 className="queue-page__status-title">대기열 현황</h3>
            <p className="queue-page__status-text">현재 대기열 인원: {queueSize}명</p>
            {typeof readyCapacity === 'number' && (
              <p className="queue-page__status-text">동시 처리 가능 인원: {readyCapacity}명</p>
            )}
            {typeof waitingAhead === 'number' && (
              <p className="queue-page__status-text">내 앞 순번: {waitingAhead}명</p>
            )}
            {typeof position === 'number' && (
              <p className="queue-page__status-position">내 순번: {position}번</p>
            )}
