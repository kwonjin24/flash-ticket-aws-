...
} from '@queue/application/services/queue-ticket.service';
  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async joinQueue(
    ...
    try {
      ...
      // this.logger.log(
      //   `Client ${client.id} joined queue for event ${targetEventId} with ticket ${ticketId}`,
      // );
  ...
  private async emitStatus(
    ...
    try {
      ...
      // this.logger.verbose(
      //   `Emitting status for ticket ${context.ticketId}: state=${status.state}, position=${status.position}, queueSize=${queueSize}`,
      // );
